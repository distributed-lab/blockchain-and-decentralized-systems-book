...
> **Внимание!_** *Выглядит довольно громоздко, сложно и пугающе, однако ниже мы рассмотрим каждый шаг взаимодействия и детально опишем, что на самом деле происходит, таким образом, что по окончании раздела читатель будет полностью понимать, как работают двунаправленные платежные каналы в Lightning Network.*
...
> _Шаг 2_. Алиса создает транзакцию №1 с двумя условиями траты монет: 1) Бобу, если он знает секрет; 2) Алисе, если следующая транзакция будет подписана ими двумя. После этого транзакция отправляется Бобу, для того чтобы он согласился с указанными условиями.
...
> _Шаг 7_. Боб создает транзакцию №3 с двумя условиями траты монет: 1) Алисе, если она знает секрет; 2) Бобу, если следующая транзакция будет подписана ими двумя. После этого транзакция отправляется Алисе, чтобы та согласилась с указанными условиями.
